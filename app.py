...
# def compare_star_types():
#     text("# Star Type Comparison Tool")
    
#     # Get list of unique star types and ensure they're strings for display and selection
#     star_types = sorted(df["Star Type"].unique().tolist())
#     star_type_labels = {
#         0: "Brown Dwarf",
#         1: "Red Dwarf",
#         2: "White Dwarf",
...
#     # Create checkboxes for star types with descriptive labels
#     selected_types = []
#     for i, star_type in enumerate(star_types):
#         label = f"{star_type_labels.get(star_type, f'Type {star_type}')} (Type {star_type})"
#         # Default first two types to checked
#         if checkbox(label, default=(i < 2)):
#             selected_types.append(star_type)
...
#     properties = ["Temperature", "Luminosity", "Radius"]
    
#     for prop in properties:
#         text(f"### {prop} Comparison")
...
#             y=prop, 
#             color="Star Type",
#             title=f"{prop} Distribution by Star Type",
#             points="all",  # Show all points
#             notched=True,  # Show notched box plot for confidence interval
#             category_orders={"Star Type": [str(t) for t in sorted(selected_types)]}  # Ensure proper order
#         )
...
#             yaxis_title=prop,
#             showlegend=(prop == "Temperature"),  # Only show legend for first plot
#             xaxis_type='category'  # Explicitly set x-axis to categorical
#         )
        
#         # Use log scale for luminosity
#         if prop == "Luminosity":
#             fig.update_layout(yaxis_type="log")
        
#         plotly(fig)
        
#         # Add a scatter plot version for clearer view of individual data points
#         if prop in ["Temperature", "Luminosity"]:
#             text(f"### Individual {prop} Values by Star Type")
#             scatter_fig = px.strip(
#                 compare_df,
#                 x="Star Type",
#                 y=prop,
#                 color="Star Type",
#                 title=f"Individual {prop} Values by Star Type",
...
#             if prop == "Luminosity":
#                 scatter_fig.update_layout(yaxis_type="log")
                
#             plotly(scatter_fig)
    
#     # Statistics table for comparison
#     text("### Statistical Comparison")
    
#     # Create a more readable stats table
#     stats_tables = []
    
#     for prop in properties:
#         # Calculate statistics for each property and star type
#         stats = compare_df.groupby("Star Type")[prop].agg([
#             ("Count", "count"),
#             ("Minimum", "min"),
#             ("Mean", "mean"),
#             ("Median", "median"),
...
#         stats["Star Classification"] = stats["Star Type"].map(
#             {str(k): v for k, v in star_type_labels.items()}
#         )
        
#         # Reorder columns to show classification first
#         stats = stats[["Star Type", "Star Classification", "Count", "Minimum", "Mean", "Median", "Maximum", "Std Dev"]]
        
#         stats_tables.append(stats)
        
#     # Display each property's stats in a separate table
#     for i, prop in enumerate(properties):
#         text(f"#### {prop} Statistics")
